...
<section class="business-section-post" id="section3">
    <div class="container section3-container">
        ...
        @if ($posts->isEmpty())
            <div class="text-center" style="color: black;">No posts available</div>
        @else
                ...
                @foreach ($posts as $post)
                    <div class="col mb-4">
                        <div class="card h-100 animate-on-scroll">
                            ...
                            @endphp
                            @if ($firstImage)
                                <img src="{{ $firstImage }}" class="card-img-top" alt="Business Image" onclick="openFullScreen('{{ route('businessPost', ['id' => $post->id]) }}')">
                            @endif
