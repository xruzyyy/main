            ...
            <div class="business-meta">
                <p class="postText"><strong>Type:</strong> {{ $post->type }}</p>
                 ...
                 <p class="card-text">
                    ...
                    @if ($post->monday_open && $post->monday_close)
                        <span><strong>Monday:</strong> {{ date('h:i A', strtotime($post->monday_open)) }} -
                            {{ date('h:i A', strtotime($post->monday_close)) }}</span><br>
                    @endif

                    @if ($post->tuesday_open && $post->tuesday_close)
                        <span><strong>Tuesday:</strong> {{ date('h:i A', strtotime($post->tuesday_open)) }}
                            - {{ date('h:i A', strtotime($post->tuesday_close)) }}</span><br>
                    @endif

                    @if ($post->wednesday_open && $post->wednesday_close)
                        <span><strong>Wednesday:</strong>
                            {{ date('h:i A', strtotime($post->wednesday_open)) }} -
                            {{ date('h:i A', strtotime($post->wednesday_close)) }}</span><br>
                    @endif

                    @if ($post->thursday_open && $post->thursday_close)
                        <span><strong>Thursday:</strong>
                            {{ date('h:i A', strtotime($post->thursday_open)) }} -
                            {{ date('h:i A', strtotime($post->thursday_close)) }}</span><br>
                    @endif

                    @if ($post->friday_open && $post->friday_close)
                        <span><strong>Friday:</strong> {{ date('h:i A', strtotime($post->friday_open)) }} -
                            {{ date('h:i A', strtotime($post->friday_close)) }}</span><br>
                    @endif

                    @if ($post->saturday_open && $post->saturday_close)
                        <span><strong>Saturday:</strong>
                            {{ date('h:i A', strtotime($post->saturday_open)) }} -
                            {{ date('h:i A', strtotime($post->saturday_close)) }}</span><br>
                    @endif

                    @if ($post->sunday_open && $post->sunday_close)
                        <span><strong>Sunday:</strong> {{ date('h:i A', strtotime($post->sunday_open)) }} -
                            {{ date('h:i A', strtotime($post->sunday_close)) }}</span><br>
                    @endif
                </p>
